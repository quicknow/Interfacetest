...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format">
	...
	<xsl:template match="/">
		<html>
			...
			<head>
				<title>WSDL Validation Report</title>
				<style>
body
{ background-color: #d4d4d4; background-image: none; background-repeat: repeat-x; margin: 0; padding: 0; overflow: auto }table.full { width: 100%; height: 100% }table.width_full { width: 100%; height: auto }td.main { padding: 5px 10px 10px; vertical-align: top }td.main_frame { background-color: white; padding: 10px 14px; vertical-align: top; border: solid 1px #8e001d }td.top { color: white; font-size: 11px; font-family: Arial, Helvetica, sans-serif; font-weight: bold; background-color: #8e001d; text-transform: capitalize; padding-left: 10px; height: 18px }td.under_top { color: #273236; font-size: 12px; font-family: Arial, Helvetica, sans-serif; text-transform: capitalize; padding-left: 10px; height: 20px }td { font-size: 11px; font-family: Verdana, Arial, Helvetica, sans-serif }p { font-size: 11px; font-family: Verdana, Arial, Helvetica, sans-serif }h1  { font-size: 18px; font-family: Verdana, Arial, Helvetica, sans-serif; line-height: 20px; margin-bottom: 10px }h2 { font-size: 14px; font-family: Verdana, Arial, Helvetica, sans-serif; line-height: 18px; margin-bottom: 10px }h3  { font-size: 12px; font-weight: bold; line-height: 14px; margin-bottom: 10px }.wingding2  { font-size: 18px; font-family: "Wingdings 2"; mso-ascii-font-family: Times New Roman; mso-char-type: symbol; mso-hansi-font-family: Times New Roman; mso-symbol-font-family: Wingdings 2  }a { color: #669 }a:hover { color: #8e001d }td.table_head { color: #485464; font-weight: bold; padding: 2px 4px; height: 4px; vertical-align: top; border-top: 2px solid #a3a9b1; border-bottom: 2px solid #a3a9b1 }td.table_head_mid { color: #485464; font-weight: bold; text-align: center; padding: 2px 4px; width: 80px; vertical-align: top; border-top: 2px solid #a3a9b1; border-bottom: 2px solid #a3a9b1 }td.table_lightrow 
...
ow { background-color: #e6e6e6; padding: 3px 4px; height: 20px; vertical-align: top }td.table_no_midrow { background-color: #e6e6e6; text-align: right; padding: 3px 35px 3px 4px; height: 20px; vertical-align: top }td.table_end { border-top: 2px solid #a3a9b1 }.red { color: #c00 }.orange { color: #f60 }
			...
			<body bgcolor="#ffffff">
				<table class="full" border="0" cellspacing="0" cellpadding="0">
					<tr>
						<td class="top">MERCURY</td>
					</tr>
					<td class="under_top">
						<b>WSDL Validation Report</b> -									<xsl:for-each select="Report">
													...
													<xsl:value-of select="position()"/>.									<a 								href="{$hrefval}"> File :									<xsl:value-of select="@name"/>
															...
															<xsl:choose>
																	...
																	<xsl:when test="msgs/msg/@severity='Warning'">
																			<b><font color="#FF9933"> - Warning</font></b>
																	</xsl:when>
																	<xsl:otherwise>
																			<b><font color="green"> - Passed</font></b>
																	</xsl:otherwise>
															</xsl:choose>								
      ...
      <xsl:for-each select="msgs">
												<table class="width_full" border="0" cellspacing="0" cellpadding="0">
													...
													<center>
														<h3>Message Information table</h3>
													</center>
													<td class="table_head_mid">Line No.</td>
													<td class="table_head_mid">Position</td>
													<td class="table_head">Message</td>
